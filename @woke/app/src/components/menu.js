...
export default function Menu({ twitterSignin, ...props }) {
	...
	const renderAuthOption = () => {
		...
		if (isLoggedIn()) {
			...
			return (
				<Link className={classes.menuItem} variant="h3" href="/" onClick={handleLogout}>
					logout
				</Link>
			);
			...
			*/
			...
			// Otherwise, we can log in if we have the user id.
			return (
				<Link className={classes.menuItem} variant="h3" onClick={handleSignOut}>
					sign out
				</Link>
			);
		} else {
			// Not logged in, and not signed in
			...
			return (
				<Link className={classes.menuItem} variant="h3" href="/">
					sign in
				</Link>
			);
		}
	...
	return (
		<>
			...
			<FlexColumn className={showMenu ? classes.showMenu : classes.hideMenu}>
				...
				>
					<Link className={classes.menuItem} variant="h3" href="/how">
						how
					</Link>
					<Link className={classes.menuItem} variant="h3" href="https://about.getwoke.me">
						about
					</Link>
					{renderAuthOption()}
				</OutsideClickHandler>
