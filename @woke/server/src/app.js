var faultMonitor = require('./lib/fault-monitor');
...
app.get('/', function (req, res, next) {
	res
		.status(200)
		.send('The server is up! Please hit one of the API endpoints to use the server');
});
...
app.use(function (err, req, res, next) {
	...
	console.error(err);
	faultMonitor.notify(err, `server(env:${$NODE_ENV})`);
