...
const initClient = async (simulate) => {
	...
	const MockTwitterOracle = new web3.eth.Contract(oracleInterface.abi, {
		data: oracleInterface.bytecode,
	}); //, {from: owner});
	...
	if (!wt_address) {
		wt = await UserRegistry.deploy({
			data: tokenInterface.bytecode,
			arguments: [to.options.address, maxSupply],
		}).send({
			from: owner,
			gas: gasLimit,
			gasPrice: gasPrice,
	...
	let getwoketoke_account = await web3.eth.accounts.wallet.add(
		'0x002e4d79c9725def6de38c72894e9339d697430242b8a60a563b0e96c39575ce'
	);

	for (let u of Object.values(users)) {
		debug.m(`With user @${u.handle}, id: ${u.id}, account: ${u.account} ...`);
		u.claimString =
			(await genClaimString(u.account, u.id, u.followers_count)) + ' garbage text';
