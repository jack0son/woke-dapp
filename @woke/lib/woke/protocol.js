...
function buildUserClaimString(web3) {
	return async (signatory, userId, followersCount, app = 'twitter') => {
		const appId = {
			default: 0,
			twitter: 10,
			youtube: 20,
		...
		return `@getwoketoke 0xWOKE:${userId},${sig},1`;
	};
}

function buildOracleClaimString(web3) {
	return async (signatory, userId, followersCount, app = 'twitter') => {
		const appId = {
			default: 0,
			twitter: 10,
